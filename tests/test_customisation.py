...
from richmin.settings import CHANGEFORM_TEMPLATES
from richmin.templatetags.richmin import get_sections
...
def test_update_site_logo(admin_client, custom_richmin_settings):
    """
    We can add a site logo, and it renders out
    """
    url = reverse("admin:index")

    custom_richmin_settings["site_logo"] = "books/img/logo.png"
...
@pytest.mark.django_db
def test_update_login_logo(client, custom_richmin_settings):
    """
    We can add a login logo, and it renders out
    """
    url = reverse("admin:login")

    custom_richmin_settings["login_logo"] = "books/img/logo-login.png"
...
def test_changeform_templates(config_value, template, admin_client, custom_richmin_settings):
    """
...
@pytest.mark.django_db
def test_changeform_template_override(admin_client, custom_richmin_settings):
    """
...
@pytest.mark.django_db
def test_changeform_template_default(admin_client, custom_richmin_settings):
    """
...
def test_changeform_section_ordering(change_form_context, order):
    ...
    admin_form.model_admin.richmin_section_order = order
